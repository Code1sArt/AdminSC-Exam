...
test("includes authenticated admin workflows and API integration", async () => {
  ...
  assert.match(app, /student-number-trigger/);
  assert.match(app, /saveStudentNumber/);
  assert.match(app, /คลิกเพื่อแก้ไขเลขที่/);
  ...
  assert.match(scoreExport, /"เลขที่"/);
  assert.match(scoreExport, /compareScoreStudents/);
  assert.match(scoreExport, /\.sort\(compareScoreStudents\)/);
